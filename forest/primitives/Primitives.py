...
class AggregateSumPrim(Primitive):
    ...
    def __call__(self, *args):
        ...
        if Config.engine.engine_type != "SparkEngine":
            ...
            print("\tRunning prime(AggregateSumPrim) on Spark")
            zone_value_count_rdd = Config.inputs[0];
            partial_sum_rdd = zone_value_count_rdd.reduceByKey(lambda x, y: (x[0] + y[0], x[1] + y[1]));
            return partial_sum_rdd;
...
class AveragePrim(Primitive):
    ...
    def __call__(self, sums = None):
        ...
        if Config.engine.engine_type != "SparkEngine":
            ...
            print("\tRunning prime(AveragePrim) on Spark")
            partial_sum_rdd = Config.inputs[0]
            zonal_avg = partial_sum_rdd.map(lambda x: (x[0], x[1][0] / float(x[1][1]))).collectAsMap()
            return zonal_avg
...
class PartialSumRasterizePrim(Primitive):
    ...
    def __call__(self, zone = None, data = None):
        ...
        if Config.engine.engine_type != "SparkEngine":
            ...
            #print("zone.data = ", zone.data)
            #print("data.x = ", data.x)
            #print("data.y = ", data.y)
            #print("data.h = ", data.h)
            #print("data.cellsize = ", data.cellsize)
    ...
    #         print(out_kv)
            ...
            print("\tRunning prime(PartialSumRasterize) on Spark")

            print ("\t-> adding count to each zone-value pair")
            zone_data_rdd = Config.inputs[0]
            zone_value_count_rdd = zone_data_rdd.mapValues(lambda x: (x, 1))
            return zone_value_count_rdd;
